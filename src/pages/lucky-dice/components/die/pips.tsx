...
function CentrePips({ value, pipStyle }: CenterPipsProps) {
  ...
  return (
    <div className={classes.container}>
      ...
      <Pip on={normalizedValue >= 8 && normalizedValue < 10} row={1} col={2} pipStyle={pipStyle} />
      ...
      <Pip on={normalizedValue >= 8 && normalizedValue < 10} row={3} col={2} pipStyle={pipStyle} />
      <Pip on={normalizedValue >= 2} row={3} col={3} pipStyle={pipStyle} />
    </div>
  )
}

export default function Pips({ value, isExtended, pipStyle = 'solid' }: PipsProps) {
  if (!isExtended) {
    return (
