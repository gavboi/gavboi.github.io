...
export default function PlayWindow({
  ...
  const faces: number[] = Array.from(
    { length: numberOfFaces },
    (_, i) => i + 1
  );
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [_, setRollTimes] = useState<number[]>(
    Array.from({ length: numberOfDice }, () => 0)
  );
  ...
  const handleDieClick = (i: number) => {
    setRollTimes((prev) => {
      ...
      // Achievement check
      const rollingNowCount = newRollTimes.filter(
        (time) => time >= Date.now() - rollTimeMs
      ).length;
  ...
  return (
    <div className={classes.root} style={themeStyle}>
      ...
      <div className={classes.noticesContainer}>
        {notices.map((notice) => (
          <NoticeText key={notice.id} notice={notice} setNotices={setNotices} />
        ))}
      </div>
    </div>
  );
