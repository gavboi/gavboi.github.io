...
import {
  ACHIEVEMENTS,
  DEFAULT_ROLL_TIME_MS,
  EASY_MODE_FACE_COUNT,
  HARD_MODE_FACE_COUNT,
  MAX_NOTICES,
  ROLL_TIME_REDUCTION_PER_UPGRADE_MS,
  UPGRADES,
} from "./constants";
...
function LuckyDicePageContent() {
   ...
   */
  const setAchievementUnlocked = useCallback(
  ...
  useEffect(() => {
    ...
    for (const roll of pendingRolls) {
      ...
      if (roll === luckyNumber) {
        ...
        const pointsGained =
          (1 + upgradeCount["higher-payout"]) *
          (upgradeCount["streak-multiplier"] + 1) ** (nextStreak - 1);
    ...
    if (nextLastThree.length === 3 && nextLastThree.every((v) => v === 6)) {
    ...
    if (
      nextRollCounts
        .slice(0, isHardMode ? HARD_MODE_FACE_COUNT : EASY_MODE_FACE_COUNT)
        .every((v) => v > 0)
    ) {
  ...
  const handleRestartGame = (clearFunc: () => void) => {
    clearFunc();
    setNotices([
      { id: `${noticeId.current++}`, text: "Restarted Game!", animate: true },
    ]);
    setAltScreen(null);
  };
    ...
    >
        ...
        <>
          <div className={classes.playWindow}>
            <PlayWindow
              ...
              dieDesign={
                achievementsUnlocked["have-winner"]
                  ? "gold"
                ...
                <div key={key} className={classes.upgradeList}>
                  {isUpgradeUnlocked(UPGRADES[typedKey]) && !winnerInEasy && (
                    <ShopItem
                      ...
                      counter={UPGRADES[typedKey].countDisplayConversion(
                        upgradeCount[typedKey]
                      )}
      ...
      <div className={classes.menuBar}>
        <div onClick={handleTrophyClick} className={classes.clickable}>
          <FontAwesomeIcon
            icon={faTrophy}
            className={`${classes.menuIcon} ${altScreen === "achievements" ? classes.menuIconSelected : ""}`}
          />
        </div>
        {upgradeCount["stats"] > 0 && (
          <div onClick={handleStatsClick} className={classes.clickable}>
            <FontAwesomeIcon
              icon={faChartBar}
              className={`${classes.menuIcon} ${altScreen === "stats" ? classes.menuIconSelected : ""}`}
            />
          </div>
        )}
        <div onClick={handleGearClick} className={classes.clickable}>
          <FontAwesomeIcon
            icon={faGear}
            className={`${classes.menuIcon} ${altScreen === "settings" ? classes.menuIconSelected : ""}`}
          />
        </div>
        <div onClick={handleInfoClick} className={classes.clickable}>
          <FontAwesomeIcon
            icon={faCircleInfo}
            className={`${classes.menuIcon} ${altScreen === "info" ? classes.menuIconSelected : ""}`}
          />
        </div>
      </div>
    </div>
